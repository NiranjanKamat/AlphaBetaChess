...
public class AlphaBetaChess {
	...
	public static void main(String[] args) {
		...
		while (!"a".equals(chessBoard.get(kingPositionL / 8, kingPositionL % 8))) {
			kingPositionL++;
	...
	public static void makeMove(String move) {
		if (move.charAt(4) == 'Z') {
			...
			chessBoard.set(
					...
					chessBoard.get(Character.getNumericValue(move.charAt(0)),
							Character.getNumericValue(move.charAt(1))));
			chessBoard.set(Character.getNumericValue(move.charAt(0)),
					Character.getNumericValue(move.charAt(1)), " ");
			if ("A".equals(chessBoard.get(Character.getNumericValue(move.charAt(2)),
					Character.getNumericValue(move.charAt(3))))) {
	...
	public static void undoMove(String move) {
		if (move.charAt(4) == 'Z') {
			...
			chessBoard.set(
					...
					Character.getNumericValue(move.charAt(1)),
					chessBoard.get(Character.getNumericValue(move.charAt(2)),
							Character.getNumericValue(move.charAt(3))));
			chessBoard.set(Character.getNumericValue(move.charAt(2)),
					Character.getNumericValue(move.charAt(3)),
					String.valueOf(move.charAt(4)));
			if ("A".equals(chessBoard.get(Character.getNumericValue(move.charAt(0)),
					Character.getNumericValue(move.charAt(1))))) {
	...
	public static String posibleP(int i) {
		...
		for (int j = -1; j <= 1; j += 2) {
			try {// capture
				if (Character.isLowerCase(chessBoard.get(r - 1, c + j).charAt(0))
						&& i >= 16) {
	...
	public static String posibleR(int i) {
		...
		for (int j = -1; j <= 1; j += 2) {
			try {
				...
				if (Character.isLowerCase(chessBoard.get(r, c + temp * j).charAt(0))) {
					oldPiece = chessBoard.get(r, c + temp * j);
					chessBoard.set(r, c, " ");
					chessBoard.set(r, c + temp * j, "R");
					if (kingSafe()) {
			...
			try {
				...
				if (Character.isLowerCase(chessBoard.get(r + temp * j, c).charAt(0))) {
					oldPiece = chessBoard.get(r + temp * j, c);
					chessBoard.set(r, c, " ");
					chessBoard.set(r + temp * j, c, "R");
					if (kingSafe()) {
		...
		for (int j = -1; j <= 1; j += 2) {
			for (int k = -1; k <= 1; k += 2) {
				try {
					...
					if (Character.isLowerCase(chessBoard.get(r + temp * j, c + temp * k)
							.charAt(0))) {
						oldPiece = chessBoard.get(r + temp * j, c + temp * k);
						chessBoard.set(r, c, " ");
						chessBoard.set(r + temp * j, c + temp * k, "B");
			...
			for (int k = -1; k <= 1; k++) {
				if (j != 0 || k != 0) {
					try {
						...
						if (Character.isLowerCase(chessBoard
								.get(r + temp * j, c + temp * k).charAt(0))) {
							oldPiece = chessBoard.get(r + temp * j, c + temp * k);
							chessBoard.set(r, c, " ");
							chessBoard.set(r + temp * j, c + temp * k, "Q");
							if (kingSafe()) {
	...
	public static boolean kingSafe() {
		...
		for (int i = -1; i <= 1; i += 2) {
			for (int j = -1; j <= 1; j += 2) {
				try {
					while (" ".equals(chessBoard.get(kingPositionC / 8 + temp * i,
							kingPositionC % 8 + temp * j))) {
						temp++;
					}
					if ("b".equals(chessBoard.get(kingPositionC / 8 + temp * i,
							kingPositionC % 8 + temp * j))
							|| "q".equals(chessBoard.get(kingPositionC / 8 + temp * i,
									kingPositionC % 8 + temp * j))) {
						return false;
					}
				} catch (Exception e) {
		...
		for (int i = -1; i <= 1; i += 2) {
			try {
				while (" ".equals(chessBoard.get(kingPositionC / 8, kingPositionC % 8
						+ temp * i))) {
					temp++;
				}
				if ("r".equals(chessBoard.get(kingPositionC / 8, kingPositionC % 8
						+ temp * i))
						|| "q".equals(chessBoard.get(kingPositionC / 8, kingPositionC % 8
								+ temp * i))) {
					return false;
				}
			} catch (Exception e) {
		...
		for (int i = -1; i <= 1; i += 2) {
			for (int j = -1; j <= 1; j += 2) {
				try {
					if ("k".equals(chessBoard.get(kingPositionC / 8 + i, kingPositionC
							% 8 + j * 2))) {
						return false;
					}
				} catch (Exception e) {
		...
		if (kingPositionC >= 16) {
			...
			for (int i = -1; i <= 1; i++) {
				for (int j = -1; j <= 1; j++) {
					if (i != 0 || j != 0) {
						try {
							if ("a".equals(chessBoard.get(kingPositionC / 8 + i,
									kingPositionC % 8 + j))) {
								return false;
							}
						} catch (Exception e) {
